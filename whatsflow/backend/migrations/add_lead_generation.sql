...
-- Add foreign key constraints after table creation (ultra-safe approach)
-- Only add foreign keys if both tables exist and have compatible column types

-- Check if contacts table exists and has compatible id column
SET @contacts_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES 
                       WHERE TABLE_NAME = 'contacts' 
                       AND TABLE_SCHEMA = DATABASE());

SET @contact_id_type = (SELECT DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS 
                       WHERE TABLE_NAME = 'contacts' 
                       AND COLUMN_NAME = 'id' 
                       AND TABLE_SCHEMA = DATABASE());
...
-- Only create foreign key if contacts table exists and FK doesn't exist
SET @sql = IF(@contacts_exists > 0 AND @contact_id_type = 'varchar' AND @fk_exists = 0, 
...
-- Check if business_profiles table exists and has compatible id column
SET @business_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES 
                       WHERE TABLE_NAME = 'business_profiles' 
                       AND TABLE_SCHEMA = DATABASE());

SET @business_id_type = (SELECT DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS 
                        WHERE TABLE_NAME = 'business_profiles' 
                        AND COLUMN_NAME = 'id' 
                        AND TABLE_SCHEMA = DATABASE());
...
-- Only create foreign key if business_profiles table exists and FK doesn't exist
SET @sql = IF(@business_exists > 0 AND @business_id_type = 'varchar' AND @fk_exists = 0, 
...
-- Add foreign key constraint for lead_activities (ultra-safe approach)
...
-- Add foreign key constraint for intent_keywords (ultra-safe approach)
SET @business_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES 
                       WHERE TABLE_NAME = 'business_profiles' 
                       AND TABLE_SCHEMA = DATABASE());
